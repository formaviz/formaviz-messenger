...
const inviteUser = (params, legacyToken) => {
    ...
    if (content.data == null || content.data.name == null || legacyToken == null || content.data.email == null) return new Answer('ADD_USER', 'ERROR', 'Failed to add the user to the channel');
    return getChannelIdByName(content.data, legacyToken)
        .then((res) =>{

            const data = {'token': legacyToken, 'channels': res, 'email': content.data.email};
